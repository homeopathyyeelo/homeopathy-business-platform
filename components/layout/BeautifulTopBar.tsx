...
interface BeautifulTopBarProps {
  onToggleLeftSidebar?: () => void;
  onToggleRightPanel?: () => void;
  showRightPanelToggle?: boolean;
}

export default function BeautifulTopBar({ 
  onToggleLeftSidebar, 
  onToggleRightPanel,
  showRightPanelToggle = false 
}: BeautifulTopBarProps) {
  const [selectedBranch, setSelectedBranch] = useState('Main Branch');
  const [notificationCount] = useState(5);
  const [showUserMenu, setShowUserMenu] = useState(false);

  return (
    <header className="h-16 bg-gradient-to-r from-orange-100 via-peach-100 to-orange-50 border-b border-orange-200 flex items-center px-6 gap-4 shadow-sm z-50">
      {/* Left Section */}
      <div className="flex items-center gap-4">
        ...
        <Link href="/dashboard" className="flex items-center gap-3">
          ...
          <div className="hidden md:block">
            ...
            <p className="text-xs text-gray-600">Homeopathy Business Platform</p>
          </div>
        </Link>
      ...
      <div className="flex items-center gap-3">
        {/* Quick Add */}
        <button className="p-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors shadow-md">
          <Plus className="h-5 w-5" />
        </button>

        {/* Notifications */}
        <button className="relative p-2.5 hover:bg-orange-200/50 rounded-lg transition-colors">
