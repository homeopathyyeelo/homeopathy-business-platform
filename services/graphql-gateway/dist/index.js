...
const resolvers = {
    Query: {
        ...
        orders: async (_, args, ctx) => {
            ...
            if (args.shopId)
                url.searchParams.set("shopId", args.shopId);
            if (args.customerId)
                url.searchParams.set("customerId", args.customerId);
            if (args.page)
        ...
        order: async (_, { id }, ctx) => {
            const base = process.env.ORDERS_SERVICE_URL || "http://localhost:3002";
            const res = await (0, node_fetch_1.default)(`${base}/api/orders/${id}`, {
                headers: { Authorization: ctx.req.headers.authorization || "" }
            });
            const data = await res.json();
            if (!data.success)
        ...
        product: async (_, { id }) => {
            const base = process.env.ORDERS_SERVICE_URL || "http://localhost:3002";
            const res = await (0, node_fetch_1.default)(`${base}/api/products/${id}`);
            const data = await res.json();
            return data.success ? data.data : null;
        },
        // Customers
        customers: async (_, args, ctx) => {
            const base = process.env.ORDERS_SERVICE_URL || "http://localhost:3002";
        ...
        customer: async (_, { id }, ctx) => {
            const base = process.env.ORDERS_SERVICE_URL || "http://localhost:3002";
            const res = await (0, node_fetch_1.default)(`${base}/api/customers/${id}`, {
                headers: { Authorization: ctx.req.headers.authorization || "" }
            });
            const data = await res.json();
            return data.success ? data.data : null;
        ...
        inventorySummary: async (_, { shopId }) => {
            const base = process.env.ORDERS_SERVICE_URL || "http://localhost:3002";
    ...
    Mutation: {
        ...
        createOrder: async (_, { input }, ctx) => {
            const base = process.env.ORDERS_SERVICE_URL || "http://localhost:3002";
            const res = await (0, node_fetch_1.default)(`${base}/api/orders`, {
                method: 'POST',
                headers: {
        ...
        updateOrderStatus: async (_, { id, status }, ctx) => {
            const base = process.env.ORDERS_SERVICE_URL || "http://localhost:3002";
            const res = await (0, node_fetch_1.default)(`${base}/api/orders/${id}/status`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: ctx.req.headers.authorization || ""
        ...
        createProduct: async (_, { input }, ctx) => {
            const base = process.env.ORDERS_SERVICE_URL || "http://localhost:3002";
            const res = await (0, node_fetch_1.default)(`${base}/api/products`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: ctx.req.headers.authorization || ""
        ...
        createCustomer: async (_, { input }, ctx) => {
            const base = process.env.ORDERS_SERVICE_URL || "http://localhost:3002";
            const res = await (0, node_fetch_1.default)(`${base}/api/customers`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: ctx.req.headers.authorization || ""
