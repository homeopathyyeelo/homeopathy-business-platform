...
func (s *ProductService) DeleteProduct(id string) error {
	return s.db.Model(&models.Product{}).Where("id = ?", id).Update("is_active", false).Error
}

func (s *ProductService) GetLowStockProducts() ([]models.Product, error) {
	var products []models.Product
	err := s.db.Where("stock <= min_stock AND is_active = ?", true).Find(&products).Error
	return products, err
}
...
func (s *ProductService) GetExpiringProducts(days int) ([]models.Product, error) {
	...
	err := s.db.Where("expiry_date <= ? AND expiry_date > ? AND is_active = ?", expiryDate, time.Now(), true).Find(&products).Error
	return products, err
}

func (s *ProductService) UpdateStock(productID string, quantity int) error {
	return s.db.Model(&models.Product{}).Where("id = ?", productID).Update("stock", quantity).Error
}
