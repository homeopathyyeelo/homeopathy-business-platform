...
-- NOTE: Using snake_case columns to align with existing code and helpers
...
-- RLS for products (temporary permissive to match current project defaults)
alter table public.products enable row level security;
create policy if not exists "Allow all operations on products"
  on public.products
  for all
  using (true)
  with check (true);
...
-- 2) Inventory table (batch-level stock)
...
create policy if not exists "Allow all operations on inventory"
  on public.inventory
  for all
  using (true)
  with check (true);
...
-- 3) Customers table (camelCase business fields supported but snake_case timestamps for compatibility)
create table if not exists public.customers (
  ...
  balanceType text default 'credit', -- 'credit' | 'debit'
  priceLevel text default 'A',       -- 'A' | 'B' | 'C'
  ...
  -- Timestamps expected by core helpers
  created_at timestamptz default now(),
  updated_at timestamptz default now()
);
...
create policy if not exists "Allow all operations on customers"
  on public.customers
  for all
  using (true)
  with check (true);
