...
const Customers = () => {
  const { getAll, create, update, delete: deleteCustomer } = useDatabase();
  ...
  // Calculate customer statistics and sales data
  const customersWithStats = customers.map((customer: Customer) => {
    ...
    const totalSpent = customerInvoices.reduce((sum: number, invoice: any) => sum + invoice.total, 0);
  ...
  const filteredCustomers = customersWithStats.filter((customer: any) => 
  ...
  const handleAddCustomer = async () => {
    ...
    try {
      await create('customers', formData);
      toast({
  ...
  const handleUpdateCustomer = async () => {
    ...
    try {
      await update('customers', editingCustomerId, formData);
      toast({
  ...
  const handleDeleteCustomer = async () => {
    ...
    try {
      await deleteCustomer('customers', deletingCustomerId);
      toast({
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {customers.filter((c: Customer) => c.type === 'retail').length}
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {customers.filter((c: Customer) => c.type === 'wholesale').length}
            </div>
          </CardContent>
        </Card>
      </div>
