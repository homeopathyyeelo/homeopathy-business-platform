...
const Settings = () => {
  ...
  const loadConfiguration = async () => {
    try {
      ...
      setConfig({
        company_name: configMap['company_name'] || '',
        company_address: configMap['company_address'] || '',
        company_phone: configMap['company_phone'] || '',
        company_email: configMap['company_email'] || '',
        gst_number: configMap['gst_number'] || '',
        whatsapp_api_key: configMap['whatsapp_api_key'] || '',
        kaleyra_api_key: configMap['kaleyra_api_key'] || '',
        email_api_key: configMap['email_api_key'] || '',
        facebook_access_token: configMap['facebook_access_token'] || '',
        instagram_access_token: configMap['instagram_access_token'] || ''
      });
    } catch (error) {
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      // Upsert each configuration entry
  ...
  const getConfigDescription = (key: string) => {
    const descriptions: Record<string, string> = {
      company_name: 'Company name displayed on invoices and reports',
      company_address: 'Company address for invoices and documents',
      company_phone: 'Primary contact phone number',
      company_email: 'Primary contact email address',
      gst_number: 'GST registration number',
      whatsapp_api_key: 'WhatsApp Business API key for messaging',
      kaleyra_api_key: 'Kaleyra SMS API key for SMS functionality',
      email_api_key: 'Email service API key for sending emails',
      facebook_access_token: 'Facebook Page access token for marketing',
      instagram_access_token: 'Instagram Business access token for marketing'
    };
